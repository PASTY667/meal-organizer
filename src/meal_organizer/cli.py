...
T = {
    "fr": {
        ...
        "web": "Activer la recherche web pour les recettes", "fr": "Français", "en": "Anglais",
        "saved": "Configuration enregistrée", "plan": "Plan de la semaine", "shopping": "Liste de courses",
        "meal": "Repas", "description": "Description", "ingredients": "Ingrédients", "cost": "Coût estimé",
        "stock": "Stock", "buy": "À acheter", "total": "Total courses estimé", "action": "Action",
        "help": "valider, pause, remplacer <jour>, question <jour>, quitter", "paused": "Plan mis en pause. Tu pourras reprendre avec --resume.",
        "accepted": "Plan enregistré.", "recipe": "Recette", "question": "Question", "replace_reason": "Pourquoi veux-tu remplacer ce repas ?",
        "no_plan": "Aucun plan en cours. Lance d'abord 'meal-organizer plan'.", "day": "Jour", "steps": "Étapes",
    },
    "en": {
        ...
        "web": "Enable web search for recipes", "fr": "French", "en": "English",
        "saved": "Configuration saved", "plan": "Weekly meal plan", "shopping": "Shopping list",
        "meal": "Meal", "description": "Description", "ingredients": "Ingredients", "cost": "Estimated cost",
        "stock": "Stock", "buy": "Buy", "total": "Estimated shopping total", "action": "Action",
        "help": "accept, pause, replace <day>, question <day>, quit", "paused": "Plan paused. Resume later with --resume.",
        "accepted": "Plan saved.", "recipe": "Recipe", "question": "Question", "replace_reason": "Why do you want to replace this meal?",
        "no_plan": "No active plan. Run 'meal-organizer plan' first.", "day": "Day", "steps": "Steps",
    },
}
...
def choose_llm(current: LLMConfig, language: str) -> LLMConfig:
    labels = {"fr": ("Configuration LLM", "Choisis comment générer les plans."), "en": ("LLM CONFIGURATION", "Choose how Meal Organizer should generate plans.")}
    title, subtitle = labels[language]
...
def setup() -> None:
    ...
    local = UserConfig(language=language)
    ...
    config = UserConfig(
        name=name, language=language, weekly_budget=budget, servings=servings,
        allergies=[x.strip() for x in allergies.split(",") if x.strip()],
        dislikes=[x.strip() for x in dislikes.split(",") if x.strip()],
        equipment=[x.strip() for x in equipment.split(",") if x.strip()], llm=llm,
    )
    save_config(config)
    Database()
    ok(f"{tr(config, 'saved')} → {ENV_PATH}")
...
def doctor() -> None:
    ...
    table.add_row("SQLite", "[green]OK[/green]", "Local database")
    table.add_row("Config", "[green]OK[/green]", f"{config.llm.provider} / {ENV_PATH}")
    if config.llm.provider == "openrouter":
        table.add_row("Web search", "[green]ON[/green]" if config.llm.web_search else "[dim]OFF[/dim]", "OpenRouter")
    try:
        ...
        table.add_row("LLM", "[yellow]WARN[/yellow]", str(exc)[:120])
    console.print(table)


inventory_app = typer.Typer(help="Manage inventory")
app.add_typer(inventory_app, name="inventory")


@inventory_app.command("list")
def inventory_list() -> None:
    config = load_config(); items = Database().list_inventory()
    table = Table(title="Inventaire" if config.language == "fr" else "Inventory")
    for column in (["Produit", "Quantité", "Unité", "Emplacement"] if config.language == "fr" else ["Product", "Quantity", "Unit", "Location"]):
...
def _show_plan(config: UserConfig, plan: MealPlan, shopping) -> None:
    ...
    table.add_column(tr(config, "day"), width=10)
    table.add_column(tr(config, "meal"), width=10)
    table.add_column(tr(config, "description"), ratio=2)
    table.add_column(tr(config, "ingredients"), ratio=2)
    table.add_column(tr(config, "cost"), justify="right", width=12)
    for meal in plan.meals:
        ingredients = ", ".join(f"{i.quantity:g}{i.unit} {i.name}" for i in meal.ingredients)
        cost = f"{meal.estimated_cost:.2f} €" if meal.estimated_cost is not None else "n/a"
        table.add_row(meal.day, meal.meal, f"[bold]{meal.name}[/bold]\n{meal.description}", ingredients, cost)
    console.print(table)
    shop = Table(title=tr(config, "shopping"))
    shop.add_column(tr(config, "ingredients")); shop.add_column("Qty", justify="right"); shop.add_column(tr(config, "cost"), justify="right")
    for item in shopping:
        cost = f"{item.estimated_cost:.2f} €" if item.estimated_cost is not None else "n/a"
        shop.add_row(item.name, f"{item.quantity:g} {item.unit}", cost)
    console.print(shop)
    console.print(Panel(f"[bold]{tr(config, 'total')}: {plan.shopping_cost:.2f} € / {config.weekly_budget:.2f} €[/bold]", border_style="green"))


@app.command()
def plan(resume: bool = typer.Option(False, "--resume", help="Resume the latest paused/draft plan")) -> None:
    config = load_config(); db = Database(); inventory = db.list_inventory(); provider = create_provider(config.llm)
    existing = db.load_latest_plan() if resume else None
    if existing:
        plan_id, _, meal_plan = existing
        meal_plan = price_plan(meal_plan, config, inventory)
        ok("Plan resumed." if config.language == "en" else "Plan repris.")
    else:
    ...
    while True:
        shopping = build_shopping_list(meal_plan, inventory)
        _show_plan(config, meal_plan, shopping)
        action = typer.prompt(tr(config, "action") + f" ({tr(config, 'help')})", default="accept").strip()
        parts = action.split(maxsplit=1); command = parts[0].lower()
        if command in {"accept", "valider", "ok"}:
            db.save_plan(meal_plan, "accepted", plan_id); ok(tr(config, "accepted")); return
        if command in {"pause", "pausee"}:
            db.save_plan(meal_plan, "paused", plan_id); ok(tr(config, "paused")); return
        if command in {"quit", "quitter", "q"}:
            db.save_plan(meal_plan, "paused", plan_id); return
        if command in {"replace", "remplacer"}:
            day = parts[1] if len(parts) > 1 else typer.prompt(tr(config, "day"))
            candidates = [m for m in meal_plan.meals if m.day.casefold() == day.casefold()]
            if not candidates:
                warn("Jour introuvable." if config.language == "fr" else "Day not found."); continue
            target = candidates[0]
            reason = typer.prompt(tr(config, "replace_reason"), default="")
            with console.status("Recherche d'une alternative..." if config.language == "fr" else "Finding an alternative..."):
                target_new = replace_meal(provider, config, target, inventory, reason)
                target_new.estimated_cost = None
                meal_plan.meals[meal_plan.meals.index(target)] = target_new
                meal_plan = price_plan(meal_plan, config, inventory)
            continue
        if command in {"question", "ask", "qst"}:
            day_and_question = parts[1] if len(parts) > 1 else typer.prompt(tr(config, "day"))
            day, _, question = day_and_question.partition(" ")
            target = next((m for m in meal_plan.meals if m.day.casefold() == day.casefold()), None)
            ...
            answer = provider.generate(build_question_request(config, target, question)); console.print(Panel(answer, title=tr(config, "question"), border_style="cyan")); continue
...
def recipe(day: str) -> None:
    ...
    console.print(table)
    steps = "\n".join(f"[bold]{i}.[/bold] {step}" for i, step in enumerate(generated.steps, 1))
    console.print(Panel(steps, title=tr(config, "steps"), border_style="yellow"))
...
def cook(day: str) -> None:
    ...
    if not saved: fail(tr(config, "no_plan")); raise typer.Exit(1)
    plan_id, _, meal_plan = saved; stored = db.load_recipe(plan_id, day)
    if not stored:
        recipe(day); stored = db.load_recipe(plan_id, day)
...
def price(product: str, quantity: float = typer.Option(1, "--quantity", "-q"), unit: str = typer.Option("unit", "--unit", "-u")) -> None:
    ...
    cost = estimate.cost_for(quantity, unit)
    table = Table(title=f"Price: {product}"); table.add_column("Observed price"); table.add_column("Cost for quantity"); table.add_column("Source"); table.add_column("Confidence")
    table.add_row(f"{estimate.price:.2f} €", f"{cost:.2f} €" if cost is not None else "n/a", estimate.source, estimate.confidence)
    console.print(table)
