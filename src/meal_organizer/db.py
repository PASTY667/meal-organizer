...
class Database:
    def __init__(self, path=DB_PATH): self.path=path; ensure_app_dir(); self._initialize()
    def connect(self):
    ...
    def list_inventory(self):
        with self.connect() as conn: rows=conn.execute("SELECT * FROM inventory ORDER BY location,name").fetchall()
    ...
    def upsert_inventory(self,name,quantity,unit,location):
        ...
        with self.connect() as conn: conn.execute("""INSERT INTO inventory(name,quantity,unit,location,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(name) DO UPDATE SET quantity=excluded.quantity,unit=excluded.unit,location=excluded.location,updated_at=excluded.updated_at""",(name.strip(),quantity,unit.strip(),location.strip(),now))
    def remove_inventory(self,name):
        with self.connect() as conn: return conn.execute("DELETE FROM inventory WHERE name=?",(name.strip(),)).rowcount>0
    def save_plan(self,plan,status="draft",plan_id=None):
        now=datetime.now(timezone.utc).isoformat(); data=plan.model_dump_json()
        with self.connect() as conn:
            if plan_id is None:
                cur=conn.execute("INSERT INTO meal_plans(status,data,created_at,updated_at) VALUES(?,?,?,?)",(status,data,now,now)); return int(cur.lastrowid)
            conn.execute("UPDATE meal_plans SET status=?,data=?,updated_at=? WHERE id=?",(status,data,now,plan_id)); return plan_id
    def load_latest_plan(self,statuses=("draft","paused")):
        placeholders=",".join("?" for _ in statuses)
        with self.connect() as conn: row=conn.execute(f"SELECT id,status,data FROM meal_plans WHERE status IN ({placeholders}) ORDER BY id DESC LIMIT 1",statuses).fetchone()
        return (int(row['id']),row['status'],MealPlan.model_validate_json(row['data'])) if row else None
    def list_recent_meal_names(self,limit_plans=4):
        with self.connect() as conn: rows=conn.execute("SELECT data FROM meal_plans WHERE status='accepted' ORDER BY id DESC LIMIT ?",(limit_plans,)).fetchall()
        names=[]
        for row in rows:
