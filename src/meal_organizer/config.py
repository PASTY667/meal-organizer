...
class UserConfig(BaseModel):
    ...
    def validate_retailer(cls, value: str) -> str:
        value = value.strip().lower()
        if value not in {"leclerc", "intermarche", "carrefour", "auchan", "generic"}:
            raise ValueError("Retailer must be leclerc, intermarche, carrefour, auchan or generic")
        return value
...
def load_config() -> UserConfig:
    ...
    try:
        return UserConfig(
            ...
            retailer=values.get("MEAL_ORGANIZER_RETAILER", "leclerc"),
...
def save_config(config: UserConfig) -> None:
    ...
    content = "\n".join([
        ...
        f"MEAL_ORGANIZER_RETAILER={config.retailer}",
